...
def process_rmd(path: str):
    ...
    rmd_text = re.sub(r"^---(.*?)^---", f"#{title}", rmd_text, count=1, flags=re.MULTILINE | re.DOTALL)

    # Strip R setup chunk
    rmd_text = re.sub(r"^```{r setup.*?^```", "", rmd_text, flags=re.MULTILINE | re.DOTALL)

    # Replace R image calls with normal markdown.
    rmd_text = re.sub('```{r.*?\nknitr::include_graphics\("(.*?)"\)\n```', "![](\g<1>)", rmd_text, flags=re.MULTILINE | re.DOTALL)

    # Replace R chunks with R syntax highlighting.
    rmd_text = re.sub(r"{r .*?}", "r", rmd_text)

    # Replace Bash cells with code block
    rmd_text = re.sub(r"```{bash.*?\n(.*?)\n```", r"```bash\n\g<1>\n```", rmd_text, flags=re.MULTILINE | re.DOTALL)
